    ...
    Mutation: {
        addRatings: authenticated(
            validateRole('coordinator')(
                async (
                    ...
                    await sendEmails( process.env.COORDINATOR_EMAIL, 
                        process.env.COORDINATOR_PASS ,userExists.email, 
                        'Trainee','This is to inform you that , new ratings are out now !','Dear Trainee')
        ...
        approveRating: authenticated(
            validateRole('admin')(async (root, { user, sprint }) => {
              ...
              updatedData[0].professionalRemark == 'no remark'
                ...
                await TempData.deleteOne({ sprint: sprint, user: user })
                await sendEmails( process.env.ADMIN_EMAIL,process.env.ADMIN_PASS ,userToNotify[0].email, 
                    'Trainee ratings',`The updates for ${userToNotify[0].email} has been approved, check new ratings `,
                    'Dear Trainee')
                return update
            })
        ),
        ...
        rejectRating: authenticated(
            validateRole('admin')(async (root, { user, sprint }) => {
                ...
                await sendEmails( process.env.ADMIN_EMAIL,process.env.ADMIN_PASS ,findCoordinatorEmail[0].email, 'Trainee ratings',`The updates for ${userX[0].email} has been rejected `,'Dear Trainee')
