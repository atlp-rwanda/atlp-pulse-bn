...
import { sendEventCancellations, sendEventInvitations, sendInvitationCancellations } from '../helpers/sendEventEmails';
...
const validateDates = (start: string, end: string): string | null=>{
  ...
  const dateRegex = /^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])T([01]\d|2[0-3]):(00|[0-5]\d):(00|[0-5]\d).\d{3}Z$/
...
const validateTime = (timeToStart: string, timeToEnd: string): string | null=>{
  ...
  if(parseInt(timeToStartArr[0]) > parseInt(timeToEndArr[0])) return "Event should start before it ends"

  if(parseInt(timeToStartArr[0]) === parseInt(timeToEndArr[0])){
    if(parseInt(timeToStartArr[1]) > parseInt(timeToEndArr[1])) return "Event should start before it ends"
  }
...
const eventResolvers: any = {
  Query: {
    getEvents: async (_: any, { authToken }: { authToken: string }) => {
      ...
      return await Event.find({ $or :[{ user: userId }, { invitees: {$elemMatch: { email: user.email, status: INVITEE_STATUS.ACCEPTED }}}]});
    },
    ...
    getAcceptedEvents: async (_: any, { authToken }: { authToken: string }) => {
      ...
      const events = await Event.find({ invitees: { $elemMatch: { email: user.email, status: INVITEE_STATUS.ACCEPTED } } })
      return events;
    },
    getEvent: async (_: any, { eventId, authToken }: { eventId: string, authToken: string }) => {
      const { userId } = decodeAuthHeader(authToken);
      const user = await User.findById(userId)
      if (!user) {
        throw new GraphQLError("No such user found", {
  ...
  Mutation: {
    async createEvent(
      _: any,
      { title, hostName, start, end, timeToStart, timeToEnd, authToken, orgToken, invitees = [] }: any
    ) {
      ...
      const validUsers = await User.find({ email: { $in: invitees }, organizations: { $in: [org.name] } })
      const validEmails = validUsers.map((user)=>user.email)
      const invalidEmails = invitees.filter((email: string)=> !validEmails.includes(email))
      if (invalidEmails.length) {
        throw new GraphQLError(`Invitees contain invalid users ${invalidEmails.toString()}`, {
          extensions: {
            code: "FORBIDDEN"
          }
        })
      }
      const duplicates = invitees.filter((email: string,index: number)=> invitees.indexOf(email) !== index)
      if (duplicates.length) {
        throw new GraphQLError(`Invitees contain duplicated users ${duplicates.toString()}`, {
          extensions: {
            code: "FORBIDDEN"
          }
        })
      ...
      event.invitees.forEach(async invitee => {
        await sendEventInvitations(invitee.email, event.id, event.title, event.hostName, event.start, event.end, event.timeToStart, event.timeToEnd)
      })
    ...
    async editEvent(
      _: any,
      { eventId, title, hostName, start, end, timeToStart, timeToEnd, authToken, orgToken, invitees = [] }: any
    ) {
      const decoded = decodeAuthHeader(authToken);
      const user = await User.findById(decoded.userId)
      if (!user) {
      ...
      const newInvitees = invitees.filter((email: string) => !oldInvitees.includes(email))
      const excludedInvitees = oldInvitees.slice(1).filter((email: string) => !invitees.includes(email))
      ...
      event.invitees.push(...newInvitees.map((email: string) => {
        return {
          email: email,
          status: INVITEE_STATUS.PENDING
        }
      ...
      event.invitees = event.invitees.filter(invitee => !excludedInvitees.includes(invitee.email))
      await event.save()

      if (newInvitees.length) {
        newInvitees.forEach(async (email: string) => {
          await sendEventInvitations(email, event.id, event.title, event.hostName, event.start, event.end, event.timeToStart, event.timeToEnd)
        })
      }
    ...
    async respondToEventInvitation(
      ...
      const existingInvitee = event.invitees.find(invitee => invitee.email === user.email)
      if (!existingInvitee) {
